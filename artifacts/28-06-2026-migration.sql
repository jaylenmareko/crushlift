-- Train Maxxing — Migration (DRAFT — NOT YET APPLIED)
-- Target: live trainmaxxing DB (project ref cheanydnmvqdvsexxdav)
-- Apply via: Supabase dashboard → SQL editor
-- Run both sections. They are safe to re-run (idempotent).

-- ── 1. Belt ranks on profiles ────────────────────────────────────────────────
-- Stores computed belt tier per lift so leaderboards can show ranks without
-- re-querying every user's pr_verifications.
-- Shape: { "Bench Press": "Lifter", "Squat": "Iron", ... }
-- Populated by /api/compute-rank (called fire-and-forget after verified PRs).

alter table public.profiles
  add column if not exists belt_ranks jsonb default '{}';

-- ── 2. Battles + friendships ─────────────────────────────────────────────────
-- Full schema is in artifacts/17-06-2026-battles-schema-draft.sql.
-- Run that file in full after this one.
...
-- ── 3. profiles RLS — allow reading other users' public fields ───────────────
-- Leaderboards need to read username + weight + belt_ranks across users.
-- This policy adds a public read on those fields (username, weight, belt_ranks only).
-- If you already have a restrictive select policy, drop it first:
--   drop policy "<existing policy name>" on public.profiles;
...
-- ── Rollback ─────────────────────────────────────────────────────────────────
-- alter table public.profiles drop column if exists belt_ranks;
-- drop view  if exists public.user_records;
-- drop table if exists public.battles;
-- drop table if exists public.friendships;
-- drop policy "profiles: anyone can read public fields" on public.profiles;
