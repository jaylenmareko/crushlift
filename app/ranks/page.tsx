...
export default function RanksPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      if (!user) return
      // Count pending and approved PRs for banner
      const { data: allPrs } = await supabase
        .from('pr_verifications')
        .select('exercise_name, declared_weight, declared_reps, created_at, verified')
        .eq('user_id', user.id)
      if (allPrs) {
        setPendingPrs(allPrs.filter(p => !p.verified).length)
        setApprovedPrs(allPrs.filter(p => p.verified).length)
      }
      // Belts read from the user's best VERIFIED PR per lift
      const prs = allPrs?.filter(p => p.verified) ?? []
      if (prs.length) {
  ...
  const { openLogPr, modals: prModals } = usePrLogger(liftData, (liftName, weight, reps, verified) => {
    if (verified) {
      // AI verified — update belt immediately
      ...
      // Manual review mode — increment pending count, don't touch belts yet
      setPendingPrs(prev => prev + 1)
    }
  })
  ...
  return (
    <div className="mobile-container flex flex-col min-h-dvh bg-[#0D0D0F] has-bottom-nav overflow-hidden">
      ...
      {(pendingPrs > 0 || approvedPrs > 0) && (
        <div className="px-5 mb-2 flex flex-col gap-2">
          {pendingPrs > 0 && (
            <div className="flex items-center gap-2.5 bg-[#F59E0B]/10 border border-[#F59E0B]/25 rounded-xl px-3.5 py-3">
              <Clock className="w-4 h-4 text-[#F59E0B] flex-shrink-0" />
              <p className="text-xs font-semibold text-[#F59E0B]">
                {pendingPrs} PR{pendingPrs > 1 ? 's' : ''} pending review — belts update once approved
              </p>
            </div>
          )}
          {approvedPrs > 0 && pendingPrs === 0 && (
            <div className="flex items-center gap-2.5 bg-[#22C55E]/10 border border-[#22C55E]/25 rounded-xl px-3.5 py-3">
              <Trophy className="w-4 h-4 text-[#22C55E] flex-shrink-0" />
              <p className="text-xs font-semibold text-[#22C55E]">
                {approvedPrs} PR{approvedPrs > 1 ? 's' : ''} approved — belts updated
              </p>
            </div>
          )}
        </div>
